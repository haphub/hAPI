...
typedef struct motor{
  ...
  float velocity;
  float velocity_last; 
  float velocity_last_last; 
  float velocity_filtered; 
  float velocity_filtered_last; 
  float acceleration; 
...
void read_encoders(actuator *m1, actuator *m2, actuator *m3, actuator *m4, int number, byte address, byte actuators[], byte reply_code);
 ...
 */
 void initialize_actuator(actuator *mtr, byte parameters[], int pwm, int dir, int enc1, int enc2){
  ...
  mtr->previous_position = mtr->Enc->read();
  mtr->current_position = mtr->Enc->read();
  mtr->velocity = 0;
}
...
void create_torque(actuator *mtr, float torque){
  ...
  if(mtr->velocity_filtered <= 0){
  torque = torque - b*mtr->velocity_filtered+m*mtr->acceleration; 
  }
  else{
  torque = torque - b*mtr->velocity_filtered+m*mtr->acceleration; 
  }
...
float read_encoder(actuator *mtr){
  ...
  mtr->previous_position = mtr->current_position;
  mtr->current_position = mtr->Enc->read();

  mtr->velocity = (mtr->current_position - mtr->previous_position)/0.00005; //time stamp for velocity based on superloop
  mtr->velocity_filtered_last = mtr->velocity_filtered; 
  mtr->velocity_filtered = (mtr->velocity)*.5+(mtr->velocity_last)*.33+(mtr->velocity_last_last)*.17;


  mtr->velocity_last = mtr->velocity; 
  mtr->velocity_last_last = mtr->velocity_last; 

  mtr->acceleration = (mtr->velocity_filtered - mtr->velocity_filtered_last)/.00005; 

  th_degrees = 360.0 * mtr->current_position/mtr->Enc_resolution;
